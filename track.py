...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # Stream results/show vid
            im0 = annotator.result()
            if show_vid:
                global count_Mobil
                # color=(0,255,0) #warna hijau
                color=(232,144,156) #warna ungu
                color_text=(0,0,0) #warna ungu
                #garis horizontal
                start_point = (0, h-350) 
                end_point = (w, h-350)
                #garis vertikal
...
def count_obj(box,w,h,id,cls):
    ...
    if cls == 0: #motor
        if int(box[1]+(box[3]-box[1])/2) > (h-350): #ini untuk horizontal
...
if __name__ == '__main__':
    ...
    # Menggunakan weight yang telah dilatih pada yolov5
    parser.add_argument('--classes', nargs='+', type=int, default=[0,1], help='filter by class: --class 0, or --class 16 17')
